...
local function CanCarryItem(source, itemName, itemQuantity)
	if Config.OxInventory then
		...
		DebugPrint("You need to use ox_inventory.")
	end
end
...
local function OpenShop(shopId)
    ...
    elseif job and job.name == shopData.Potatis144_JobLock then
        OpenShopUI(shopData)
    elseif gang and gang.name == shopData.Potatis144_GangLock then
        OpenShopUI(shopData)
    else
        Potatis144_ServerNotify(Locales.Notification.WrongJob, "error")
    end
end
